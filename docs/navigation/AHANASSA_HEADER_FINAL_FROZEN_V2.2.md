...
## Final Frozen Header — Version 2.2
...
# 82. Historical Checkpoint — Version 2.1 — SUPERSEDED

**Historical checkpoint:** AHAN ASA HEADER FINAL FROZEN V2.1 — CONSOLIDATED ACCESSIBILITY, UX & CONVERSION HARDENING — APPROVED.

Version 2.1 remains preserved for architectural history and is superseded only by the Version 2.2 shared-Button adoption rules below.

All non-Button Header architecture frozen in V2.1 remains in force.


---

# 83. Version 2.2 — Shared Button Component Adoption

Version 2.2 does not reopen Header Information Architecture, routes, Product/Services projection rules, responsive shell geometry, disclosure behavior, mobile drawer hierarchy, phone utility, language utility, sticky behavior, or Primary CTA wording.

It changes Button ownership.

The Header no longer owns an independent Primary CTA visual specification.

Authoritative source:

```text
AHANASSA_BUTTON_COMPONENT_FINAL_FROZEN_V1.0.md
```

---

## 83.1 Single source of truth

The Header Primary CTA:

**ارسال لیست خرید**

must consume the shared Button Component:
...
- placement;
- width available inside the Header;
- surrounding layout gap;
- responsive visibility;
- drawer composition.

Header must not independently redefine:
...
- semantic Primary fill.

---

# 84. Superseded Header CTA Geometry

The earlier Header-only statement in §46.12:

```text
Target height: 48px
```

remains numerically correct but is no longer independently owned by Header.

Its authority is superseded by the shared Button Component V1.0 contract:
...
Any future Button geometry change must be versioned in the Button Component document first.

Header may not silently diverge.

---

# 85. Header Primary CTA Usage Contract

## 85.1 Desktop

Desktop Primary CTA uses:
...
It remains the only dominant solid-filled action in the Header.

## 85.2 Mobile drawer

Drawer Primary CTA uses the same shared Primary Button.
...
This width change does not create a new visual Button variant.

## 85.3 Compact sticky state

The compact Header must not shrink the Button below the shared 48px height merely because the Header shell reduces toward ~68px.

The Header shell must accommodate the shared Button safely.

---

# 86. Utility Controls Are Not Buttons

The following remain outside the shared Primary/Secondary Button family:

- Phone utility;
- Language selector;
- Products disclosure chevron;
- Services disclosure chevron;
- mobile menu trigger;
- mobile close control.

These controls retain their own semantic/control contracts.

In particular:

> The phone utility must never adopt the Primary CTA visual treatment.

Phone remains a real low-weight `tel:` action.

---

# 87. Header Button Acceptance Addendum

Header V2.2 PASS requires:

- Primary CTA uses the shared Button Component;
- exact shared 48px height;
- exact shared 8px radius;
- exact shared 28px horizontal padding;
- exact shared 16px / 600 typography;
- shared hover/pressed/focus states;
- `:focus-visible` behavior comes from shared Button;
- forced-colors behavior comes from shared Button;
- reduced-motion behavior comes from shared Button;
- Header does not duplicate Button tokens locally;
- mobile drawer CTA uses the same component, with width only controlled by layout;
- Phone and Language do not visually compete as Button variants;
- the Header remains the one-dominant-filled-action environment already frozen in V2.1.

---

# 88. Version 2.2 Current Authoritative Status

**AHAN ASA HEADER FINAL FROZEN V2.2 — SHARED BUTTON COMPONENT ADOPTION — APPROVED**

Version 2.2 supersedes Version 2.1 only for ownership and exact implementation of the Primary CTA Button visual/interaction contract.

All other Header V2.1 architecture, accessibility, navigation, localization, responsive, interaction, data-source, and conversion rules remain in force.

Authoritative Button source:

```text
AHANASSA_BUTTON_COMPONENT_FINAL_FROZEN_V1.0.md
```

Future Header changes require an explicitly versioned Header document.
Future Button changes require an explicitly versioned Button Component document.
