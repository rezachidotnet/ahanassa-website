...
| 7 (new) | This workflow itself has never been dispatched, not even once | **Open, by design** — a dry run (invalid `deploy_ref`, then a SHA that never reached staging, confirming every assertion fails closed live in Actions) is the natural next step before the first real release at `rollout_percentage: 10`, exactly as the design document's own "Recommended Next Step" describes. Not performed by this task, which was scoped to authoring the workflow file only. |
