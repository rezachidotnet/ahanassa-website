...
**Schema:** `route_redirects` (`migrations_public/0005_homepage_projection.sql`, corrected by `migrations_public/0006_route_redirects_308.sql`) — generic across entity types (`entity_type`/`entity_id` columns, never product-specific), `(locale, old_path)` unique, `status_code IN (308, 302, 410)`, `target_path` required for 308/302 and forbidden (NULL) for 410 (CHECK constraints), full site-relative paths (e.g. `/products/old-slug`) rather than bare slugs. **308, not 301** — this is deliberate, see the HTTP semantics paragraph below.

**Canonical slug change — ATOMIC, not best-effort (code-review hardening pass, DAR-054):** `lib/catalog/editorial-repository.ts#upsertEditorialDraft` detects an actual slug change on an existing `entity_type='product'` row (never on first-time slug creation — no previous row means nothing to redirect from) and writes the `product_seo_contents` row AND the redirect row (a 308 from the previous `/products/{slug}` to the new one, plus any write-time chain collapse — `lib/catalog/route-redirects-logic.ts#planSlugChangeRedirectStatements`) in a SINGLE D1 `.batch()` call. D1's `.batch()` is one implicit transaction: either both writes land or neither does — an existing indexed canonical URL can never end up 404ing because a redirect write failed independently of the slug change that made it necessary. If the redirect cannot even be validated (a self-redirect/loop against existing rows), the whole slug change is refused outright (`{ status: "redirect_conflict" }`) before any write is attempted, rather than silently proceeding with an unprotected old URL. The actual redirect-safety decision and statement construction is pure and D1-free (`lib/catalog/route-redirects-logic.ts#planSlugChangeRedirectStatements`) — the D1-touching `lib/catalog/route-redirects.ts#buildSlugChangeRedirectStatements` is a thin wrapper around it. `app/[locale]/products/[slug]/page.tsx` checks `resolveRouteRedirect(locale, path)` before calling `notFound()` and issues a real `permanentRedirect()` (not `redirect()`) for a resolved 308.

**HTTP semantics match the stored contract exactly:** an earlier version of this feature stored `status_code = 301` but called `next/navigation`'s plain `redirect()`, which under this stack's Next.js App Router actually emits a temporary 307/303 — the database claimed "permanent" while the real response was temporary. Fixed by switching to `permanentRedirect()` (real HTTP 308) and correcting the stored contract to `308` end-to-end (schema, types, tests, this doc) — the database and the actual HTTP response must never diverge again.

**Loop/self-redirect protection:** `lib/catalog/route-redirects-logic.ts#validateRedirectInsert` rejects a self-redirect (`oldPath === targetPath`) and walks the existing chain (bounded, `MAX_HOPS = 10`) to reject a cycle — normal multi-hop chains are allowed (and immediately collapsed to one hop by the write path above), only true cycles are rejected. Fully unit-tested (`lib/catalog/route-redirects-logic.test.ts`), including the atomic statement-planning layer (`planSlugChangeRedirectStatements`) that proves a refused plan carries zero statements.
