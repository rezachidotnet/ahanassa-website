...
| `docs/GO_LIVE_READINESS.md` | root (new) | ACTIVE | Canonical readiness record for the pre-`ahanassa.com`-cutover phase: production Catalog editorial inventory/selection/publication (3 launch templates, 12 representative variants), the hreflang/canonical-URL/robots/structured-data SEO audit, the Catalog-filter conditional-narrowing fix, the RFQ form UX fixes (layout collision, RTL select clipping, required-field markers, phone-required contract change, Turnstile disabled-state UX), security/performance/Cron/Queue/DLQ verification, and the overall Go-Live gate verdict (`GO-LIVE BLOCKED` pending the UOM/Inventory/Procurement gate and a small number of other named items). Implementation touches `lib/catalog/`, `lib/metadata/`, `lib/rfq/validation.ts`, `app/[locale]/products/**`, `app/[locale]/contact/page.tsx`, `components/contact/`, `components/products/`. |
| `docs/GO_LIVE_CUTOVER_RUNBOOK.md` | root (new) | ACTIVE — documented plan only, NOT executed | Canonical Stage 2 cutover plan (final deploy, Basic Auth removal, Turnstile real-domain allowlist, Worker custom-domain attachment, DNS change, www policy, Vercel transition, post-cutover smoke tests, rollback triggers/commands, monitoring) — explicitly gated on `docs/GO_LIVE_READINESS.md` reading `GO-LIVE READY` and the UOM/Inventory/Procurement gate being resolved or owner-waived. |
| `docs/UOM_INVENTORY_PROCUREMENT_GATE.md` | root (new) | ACTIVE — OPEN gate, not implementable from this repository | Names and describes the required Odoo-side unit/inventory/procurement reconciliation gate (customer-requested UOM vs. Odoo inventory UOM, branch/sheet/meter→nominal-kg conversion, coil/bundle conversion, nominal vs. actual weight, Supplier Offer UOM, MOQ UOM, sourcing allocation, PO/receipt/Vendor Bill/Customer Invoice reconciliation, returns/cancellation, multi-supplier allocation) — an explicit, owner-mandated blocker on the overall Go-Live verdict, independent of Website code readiness. No Website code implements or resolves this. |
