...
| `docs/CATALOG_EDITORIAL_PUBLICATION.md` | root (new) | ACTIVE | Canonical for the Website-owned editorial/publication workflow sitting between Odoo's real synced commercial catalog data (`DB_PUBLIC`) and any future public catalog page: ownership split, editorial state machine, the single publication-eligibility predicate, per-locale publication, slug architecture, and the open variant-vs-template SEO granularity question. Implementation: `lib/catalog/editorial.ts`, `lib/catalog/editorial-repository.ts`. `DOCUMENT_AUDIT_REPORT.md` DAR-036. |
