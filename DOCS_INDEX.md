...
| *(referenced but absent: `ODOO_INTEGRATION.md`)* | — | MISSING, partially superseded | Its intended contract is substantially covered by `01-sources/TECHNICAL_ARCHITECTURE.md` §14 and `01-sources/DATA_ARCHITECTURE(1).md` §20–§27. The RFQ-path protocol/version/model-mapping discovery gate itself is now CLOSED — verified against the live environment 2026-08-28 (`DOCUMENT_AUDIT_REPORT.md` DAR-026, `PROJECT_OVERRIDES.md` §3, `lib/odoo/mapping.ts`). Catalog/pricing sync mapping remains open. A dedicated document authoring this content is still not written; `lib/odoo/mapping.ts` is the current source of truth for the RFQ path pending that. |
