...
**Deliberately not decided in this phase:** whether public catalog pages will be per-variant (237), per-template (13), or a hybrid — the schema supports any of the three without a further migration; route architecture is a separate, later decision. **Deliberately not built:** any admin/editorial UI, any Website authentication system, any public write endpoint — `lib/catalog/editorial-repository.ts` is a tested service layer ready for whichever of those a future task selects.
