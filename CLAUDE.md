...
`main`'s deployment-branch environment policy restricts every real `deploy-production.yml`/`deploy-staging.yml`/`verify-production.yml`/`promote-production.yml` dispatch to `feat/header-hero-integrated` — `main` is never itself a deploy source. But release *governance* — the policy that decides how risky a change is and what release path it must take — is kept discoverable here too, specifically so a session starting cold on this default branch cannot miss it:
...
That file is a synchronized copy of the authoritative policy maintained on `feat/header-hero-integrated` (kept byte-identical by a dedicated registration sync, the same pattern already used for the workflow files above — see that document's own header for its current lifecycle state). The release ledger (`docs/release/PRODUCTION_DEPLOYMENT_MANIFEST.md`) and the policy engine (`lib/ci/release-risk-classifier.ts` and siblings) are **not** duplicated here — they live only on `feat/header-hero-integrated`, since real release classification always needs the actual application diff, which does not exist on this branch. This is also why `main` needs no copy of them for `deploy-production.yml`'s release policy gate (`RELEASE_POLICY.md` §0.2): the gate reads the engine and ledger from the workflow's own commit, and every production dispatch runs from `feat/header-hero-integrated`. Check out `feat/header-hero-integrated` for those.
...
- assume a copy of `RELEASE_POLICY.md` existing here means the release policy is `ACTIVE` — check its own stated lifecycle state (`RELEASE_POLICY.md` §0) and whether the change it names has actually merged into `feat/header-hero-integrated`
- reintroduce a staging-provenance break-glass for a normal release, widen the `VALIDATED_HISTORICAL_LEDGER_APPEND` ledger exemption, or let an operator supply a promotion's `FINAL_RISK` — the three owner decisions of 2026-09-23 (`RELEASE_POLICY.md` §5.1, §7.1, §11.1) are mandatory wherever you operate from
