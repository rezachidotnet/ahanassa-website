...
**Status: Active, registered by `POLICY_BOOTSTRAP` (2026-09-22) — `docs/release/RELEASE_POLICY.md` is the authoritative human policy; `lib/ci/release-risk-classifier.ts`, `lib/ci/release-ledger.ts`, `lib/ci/emergency-rollback.ts`, and `lib/ci/policy-bootstrap.ts` are its machine enforcement.** This section governs planning or executing any staging or production release, promotion, or emergency rollback of the website — it does not govern Odoo server/module deployment (`docs/release/RELEASE_POLICY.md` §1/§18).
...
| Release governance / risk classification / production promotion / emergency rollback / CI-CD workflow changes | **Mandatory, see `CLAUDE.md` §5b:** `docs/release/RELEASE_POLICY.md` (authoritative policy), `docs/release/PRODUCTION_DEPLOYMENT_MANIFEST.md` (the ledger), `lib/ci/release-risk-classifier.ts`, `lib/ci/release-ledger.ts`, `lib/ci/emergency-rollback.ts`, `lib/ci/policy-bootstrap.ts` (machine enforcement) — supersedes `docs/release/CI_CD_POLICY.md`'s pre-production-automation description for anything the newer policy covers |
