...
- **Status:** Active — canonical control layer, project pre-implementation; homepage visual reference registered; customer account/portal future-phase architecture registered; release governance active (§5b), DAR-059/060/061 resolved 2026-09-23
- **Version:** 1.4.1
...
**Lifecycle state: `ACTIVE`** (`docs/release/RELEASE_POLICY.md` §0), effective when the **corrected** enforcement change recorded in `docs/release/GLOBAL_RELEASE_POLICY_ENFORCEMENT_IMPLEMENTATION_REPORT.md` (the original wiring **plus** the 2026-09-23 DAR-059/060/061 resolutions in its addendum) is merged into `feat/header-hero-integrated` — until that merge the live state is still `BOOTSTRAP_MERGED`. `BASE_PRODUCTION_SHA_RESOLVED: YES` (latest `STABLE_100` ledger row, first appended after `promote-production.yml` run `35719752606`). `GLOBAL_RELEASE_TIME_POLICY_ENFORCEMENT_ACTIVE: YES`: `deploy-production.yml`'s "Release policy gate" step runs `lib/ci/release-gate-cli.ts` before any production mutation — it resolves `BASE_PRODUCTION_SHA` from the ledger, classifies `BASE_PRODUCTION_SHA..deploy_ref` with `lib/ci/release-risk-classifier.ts`, requires an explicit `declared_risk`, computes `FINAL_RISK = max(declared, computed)`, and permits only the rollout that `FINAL_RISK`'s path allows (LOW/MEDIUM: 100; HIGH: 10% canary with staging provenance, then `verify-production.yml` → observation → `promote-production.yml`) — `RELEASE_POLICY.md` §0.2. The engine and ledger it uses are read from the workflow's own commit, never from the candidate. `docs/release/RELEASE_POLICY.md` is the authoritative human policy; `lib/ci/release-risk-classifier.ts`, `lib/ci/release-ledger.ts`, `lib/ci/release-gate.ts`, `lib/ci/emergency-rollback.ts`, and `lib/ci/policy-bootstrap.ts` implement the engine (`POLICY_ENGINE_IMPLEMENTED: YES`). This section governs planning or executing any staging or production release, promotion, or emergency rollback of the website — it does not govern Odoo server/module deployment (`docs/release/RELEASE_POLICY.md` §1/§18).

Three policy rules resolved by owner decision on 2026-09-23 (`DOCUMENT_AUDIT_REPORT.md` DAR-059/060/061) govern any release work from here:

- **Staging provenance is mandatory at every `FINAL_RISK`** — LOW, MEDIUM and HIGH alike (`RELEASE_POLICY.md` §5.1). `deploy-production.yml` has **no** `skip_staging_provenance` input; the gate refuses the value outright (`STAGING_PROVENANCE_REQUIRED`). `EMERGENCY_ROLLBACK` (§13) is a separate governed operation, never a staging or release-risk bypass.
- **`VALIDATED_HISTORICAL_LEDGER_APPEND`** (`RELEASE_POLICY.md` §7.1, `lib/ci/ledger-append-exemption.ts`) removes a **proven, append-only, historical** ledger row addition from risk computation, so the LOW/MEDIUM paths stay reachable. The ledger remains a `HIGH_RELEASE_PATHS` file. Any other ledger change stays HIGH; an untrustworthy one blocks the run (`LEDGER_INTEGRITY_VIOLATION`). **Operator corollary: a ledger row-append commit must change nothing else in that file** — narrative belongs in the release report.
- **Promotion propagates the release's real `FINAL_RISK`** (`RELEASE_POLICY.md` §11.1). `promote-production.yml` reads it from the bound production release-evidence artifact and requires `LOW`/`MEDIUM`/`HIGH`; an operator-supplied or hardcoded value is never accepted.
...
- reintroduce a staging-provenance break-glass for a normal release, or repurpose `EMERGENCY_ROLLBACK` as one (`RELEASE_POLICY.md` §5.1)
- widen the `VALIDATED_HISTORICAL_LEDGER_APPEND` exemption, reimplement it in YAML/bash, or apply it to any ledger change that is not a proven append-only row addition (`RELEASE_POLICY.md` §7.1)
- hardcode, default or operator-supply a promotion's `FINAL_RISK`, or edit a historical ledger row to make one agree (`RELEASE_POLICY.md` §11.1)
...
| Release governance / risk classification / production promotion / emergency rollback / CI-CD workflow changes | **Mandatory, see `CLAUDE.md` §5b:** `docs/release/RELEASE_POLICY.md` (authoritative policy, lifecycle state `ACTIVE` — §0/§0.2), `docs/release/PRODUCTION_DEPLOYMENT_MANIFEST.md` (the ledger), `lib/ci/release-risk-classifier.ts`, `lib/ci/release-ledger.ts`, `lib/ci/emergency-rollback.ts`, `lib/ci/policy-bootstrap.ts`, `lib/ci/ledger-append-exemption.ts` (the policy **engine** — `POLICY_ENGINE_IMPLEMENTED: YES`), `lib/ci/release-gate.ts`/`lib/ci/release-gate-cli.ts` (the release-time gate `deploy-production.yml` runs before any production mutation — `GLOBAL_RELEASE_TIME_POLICY_ENFORCEMENT_ACTIVE: YES`, `RELEASE_POLICY.md` §0.2) — supersedes `docs/release/CI_CD_POLICY.md`'s pre-production-automation description for anything the newer policy covers |
