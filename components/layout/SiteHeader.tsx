...
/**
 * Global site header — HEADER_NAVIGATION_SPEC.md. Structural shell only for
 * brand mark/CTA/locale switching; full disclosure-menu behavior deferred
 * (no approved sub-navigation content exists yet).
 */
    ...
    >
      <div className="container-x flex h-18 items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <nav aria-label={t.language} className="hidden items-center gap-2.5 text-xs lg:flex">
            {locales.map((code) => (
              <Link
                key={code}
                ...
                className={cn(
                  "text-muted-foreground transition-colors hover:text-navy",
                  code === locale && "text-navy font-semibold",
                )}
              >
      ...
      <div id="mobile-nav" hidden={!open} className="bg-navy fixed inset-x-0 top-18 bottom-0 z-40 lg:hidden">
        <nav aria-label={t.mobileNav} className="container-x flex flex-col pt-6">
          ...
          <Link
            href={localizedPath(locale, "/contact")}
            className="bg-copper mt-8 px-6 py-4 text-center text-sm font-semibold tracking-wide text-white"
          >
