...
The initial release is not a checkout-based online store. Cart, online payment, public inventory promises, and supplier marketplace remain out of scope unless separately approved. A customer self-service portal remains out of scope for the *current* implementation phase; its approved future architecture (not yet authorized to build) is defined in `CUSTOMER_ACCOUNT_ARCHITECTURE.md` and `CUSTOMER_PORTAL.md` — see `DECISIONS.md` ADR-017.
...
| Website customer account (future) | Website (account/session) + Odoo partner mapping | Not yet implemented — see `CUSTOMER_ACCOUNT_ARCHITECTURE.md` §3, §5 |
...
`CUSTOMER_PORTAL.md` §5 restates this section as binding for future portal-visible commercial data (quotations/orders) — it does not relax it.
...
### 12.4a Future account association (non-disruptive)

Guest RFQ submission (this section) requires no account and MUST continue to require none — see `CUSTOMER_ACCOUNT_ARCHITECTURE.md` §2. The current RFQ schema has been reviewed and confirmed to permit a future nullable customer/account relationship through an additive migration with zero disruption to existing rows (`DOCUMENT_AUDIT_REPORT.md` DAR-024; `CUSTOMER_ACCOUNT_ARCHITECTURE.md` §6). No such column is added by this section. When the account phase is scoped: current guest RFQs remain valid, the public `reference_number` is never regenerated by account linking, and Odoo synchronization state is unaffected by whether an RFQ is later linked to an account.
...
A future website Customer entity (`CUSTOMER_ACCOUNT_ARCHITECTURE.md` §5) maps to `res.partner` through this same `integration_mappings` mechanism and the same deduplication rule — it is not a new integration path. A new Odoo partner MUST NOT be created on every RFQ; synchronization MUST resolve/reuse an existing mapped partner whenever one already exists for the contact.
...
Odoo integration does not imply a customer portal; that remains out of the current implementation scope. Its approved future architecture — MVP boundary, authorization model, and no-live-Odoo-read requirement — is defined in `CUSTOMER_PORTAL.md` and `CUSTOMER_ACCOUNT_ARCHITECTURE.md`; building it still requires a separate future implementation approval (`DECISIONS.md` ADR-017).
